...
if (Test-Path "$env:USERPROFILE\.claude\statusline.mjs") {
    Write-Host "  [PASS]  Status line installed (~/.claude/statusline.mjs)" -ForegroundColor Green
} else {
    Write-Host "  [WARN]  Status line not installed - Claude Code still works" -ForegroundColor Yellow
}
...
Write-Host "    cc                  Start Claude Code (any folder, any shell)" -ForegroundColor Gray
...
Write-Host ""
Write-Host "  Defaults configured for you:" -ForegroundColor White
Write-Host "    - Auto mode is ON (auto-approves with safety checks)" -ForegroundColor Gray
Write-Host "    - Reasoning is set to xhigh (deepest persistent level)" -ForegroundColor Gray
Write-Host "    - Status line shows model, context %, git branch, 5h/7d usage" -ForegroundColor Gray
Write-Host "      (edit the CONFIG block in ~/.claude/statusline.mjs to change it)" -ForegroundColor Gray
Write-Host ""
Write-Host "  Full 'ultracode' (xhigh + auto-run workflows) can't be a" -ForegroundColor White
Write-Host "  permanent default. To turn it on for a whole session, type" -ForegroundColor Gray
Write-Host "  this inside Claude Code:" -ForegroundColor Gray
Write-Host "    /effort ultracode" -ForegroundColor Cyan
Write-Host "  Or just start any single prompt with the word 'ultracode'." -ForegroundColor Gray
Write-Host ""
pause
